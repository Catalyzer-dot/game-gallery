...
function mergeGameData(
  ...
  if (!userGames) {
    return (backendGames as UserGameWithDetails[])
      ...
      .map((backendGame) => ({
        ...
        sortOrder: backendGame.sort_order ?? 0,
        // 用户游戏列表接口会返回用户入库时间 added_at，优先用它保证分页排序稳定
        addedAt: backendGame.added_at ?? backendGame.created_at,
  ...
  for (const backendGame of backendGames) {
    ...
    const game: Game = {
      ...
      sortOrder: userGame.sort_order ?? 0,
      addedAt: userGame.added_at ?? userGame.created_at ?? backendGame.created_at,
      lastUpdated: backendGame.updated_at,
      steamUrl: backendGame.steam_url,
      coverImage: backendGame.capsule_image,
      // 优先使用后端返回的数据（如果有）
